 #ifndef GAPBS_SB_BUILDER_H
#define GAPBS_SB_BUILDER_H
...
class BuilderBase
{
  ...
  explicit BuilderBase(const CLBase &cli) : cli_(cli) 
  {
    ...
    {
      std::cout << "In-place building (-m) does not support weighted graphs" << std::endl;
      exit(-30);
    }
  }
    ...
    else // symmetrize graph by adding missing inverse edges
    {
      ...
      *neighs = static_cast<DestID_ *>(std::realloc(*neighs, newsize));
      ...
      for (NodeID_ u = 0; u < num_nodes_; u++) 
      {
        ...
        {
          DestID_ v = (*neighs)[i];
          bool inv_found = std::binary_search(*neighs + offsets[v] + invs_needed[v], *neighs + offsets[v + 1], static_cast<DestID_>(u));
          if (!inv_found) 
          {
