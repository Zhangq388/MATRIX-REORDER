#ifndef GAPBS_SB_READER_H
#define GAPBS_SB_READER_H
...
class Reader 
{
  ...
  {
    EdgeList el;
    std::string start, object, format, field, symmetry, basezero, line;

    std::getline(in, line);
    std::stringstream(line) >> start >> object >> format >> field >> symmetry >> basezero;
    ...
    bool is_basezero = (basezero == "basezero")? true : false;
    ...
    std::stringstream(line) >> m >> n >> nonzeros >> std::ws;
    if (m != n) 
    {
    ...
    while (std::getline(in, line)) 
    {
      ...
      if (read_weights) 
      {
        NodeWeight<NodeID_, WeightT_> v;
        edge_stream >> v;
        v.v = (is_basezero)? v.v : (v.v - 1);
        el.push_back(Edge(u, v));
        if (undirected)
        {
          el.push_back(Edge(v.v, NodeWeight<NodeID_, WeightT_>(u, v.w)));
        }
      } 
      else 
      {
